...
for (let row = 0; row < treeGrid.length; row++) {
    for (let col = 0; col < treeGrid[row].length; col++) {
        ...
        if (isTreeVisible(currentHeight, -Infinity, row, col, 'up') ||
...
function isTreeVisible (startingHeight, currentHeight, row, col, directionToSearch) {
    ...
    return isTreeVisible(startingHeight, treeGrid[newRow][newCol], newRow, newCol, directionToSearch);
}
